...
const AddToFavourites = async (req, res) => {
  ...
  try {
    ...
    if (!Favourite) {
      ...
      const existingItem = Favourite.items.find(
...
const GetFavourites = async (req, res) => {
  ...
  try {
    const Favourite = await favouriteModel
      .findOne({ user: Authenticated })
      .populate("items.car", "name brand price imageUrl");
    if (!Favourite) {
...
const DeleteFavourite = async (req, res) => {
  ...
  try {
    ...
    const updatedItems = Favourite.items;
    await Favourite.save();
    const updatedFavourite = await favouriteModel.findOne({
      user: Authenticated,
    });

    return res.status(200).json({
      message: "Car removed from favourites",
      items: updatedFavourite.items,
    });
  } catch (error) {
